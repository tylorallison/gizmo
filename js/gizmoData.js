...
class GizmoHandle {
    ...
    link(trunk, schema, keyer) {
        ...
        let schemas = this.node.constructor.$schema;
        if (schemas) {
            for (const agk of schemas.trunkGenDeps) {
                if (agk in this.node) this.proxy[agk] = '#autogen#';
            }
        }
        if (this.pathAutogen) {
            for (const hdl of this.constructor.eachInPath(this, (ohdl) => (ohdl.schema && ohdl.schema.autogendeps.size))) {
                for (const agk of hdl.schema.autogendeps) {
